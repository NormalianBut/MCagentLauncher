...
|---|---|---:|---:|---|---|---|---|---|
| R-001 | Instance mutation | Medium | High | Application-managed root first, canonical containment, immutable manifest plus transaction journal, no existing-instance mutation | Path-property tests, canary files, manual recovery review | Restore snapshot or remove only transaction-owned staged files | Desktop Executor maintainers | Identified |
...
| R-011 | Review/confirmation substitution | Medium | High | Bind single-use confirmation to plan, analysis, manifest, workspace, capability, presentation, and risk revisions | Mutation/replay/expiry contract tests and future end-to-end confirmation tests | Consume confirmation, return to review, preserve a redacted failure record | Desktop security maintainers | Mitigating |
| R-012 | Journal or audit ambiguity | Medium | High | Separate immutable desired state from ordered digest-chained observations; reject gaps, reordering, and invalid transitions | Deterministic sequence/state/digest-chain tests plus future torn-write fault injection | Enter recovery-required, stop automatic execution, preserve evidence for manual review | Desktop Executor maintainers | Mitigating |
